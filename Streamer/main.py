...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        connection_manager.loop = loop
        logger.info("Set connection_manager.loop to FastAPI event loop")

        # Suppress noisy Windows disconnect errors (WinError 10054) from asyncio transports
        def _loop_exception_handler(loop, context):
            exc = context.get("exception")
            msg = context.get("message", "") or ""
            # Benign: client aborted connection; common on Windows with Proactor loop
            if isinstance(exc, ConnectionResetError):
                logger.debug("Ignoring ConnectionResetError in event loop: %s", exc)
                return
            if (
                "connection lost" in msg.lower()
                or "connection was forcibly closed" in msg.lower()
            ):
                logger.debug("Ignoring loop message: %s", msg)
                return
            # Fallback to default logging
            logger.error("Unhandled event loop exception: %s", context, exc_info=exc)

        try:
            loop.set_exception_handler(_loop_exception_handler)
            logger.info(
                "Installed custom asyncio exception handler for benign disconnects"
            )
        except Exception as e:
            logger.warning(f"Could not set loop exception handler: {e}")
    except Exception as e:
    ...
    try:
        from src.services.musetalk import (
            initialize_musetalk_on_startup,
            get_musetalk_realtime_service,
        )
